...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Filters: workspace=${workspaceFilter}, parentType=${parentTypeFilter}, search=${searchQuery}, databaseId=${databaseId}`);
    ...
    let pagesWithDetails = [];
    
    // Check if we need to query a specific database
    if (databaseId) {
      ...
      try {
        ...
        if (startCursor) {
          databaseQuery['start_cursor'] = startCursor;
        }
        ...
        console.log(`Retrieved ${queryResults.results?.length || 0} pages from database`);
        ...
        for (const page of queryResults.results) {
          try {
            ...
            if (page.properties) {
              // Find a title property (Notion databases can have different property layouts)
        ...
        return new Response(
          JSON.stringify({
            ...
            next_cursor: queryResults.next_cursor || null,
            has_more: queryResults.has_more || false,
            workspace_name: workspaceName
          }),
          {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        ...
        console.error(`Error querying database ${databaseId}:`, err);
        throw err;
      }
    } else {
      // Default behavior: Fetch pages from Notion with pagination and filtering support
      const searchParams = {
        filter: {
          value: 'page',
          property: 'object'
