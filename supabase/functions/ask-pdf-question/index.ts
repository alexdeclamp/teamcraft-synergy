  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Fall back to using document context if file_url approach fails
        if (documentContext && documentContext.trim() !== '') {
          ...
          if (!fallbackResponse.ok) {
            ...
            throw new Error(`Fallback Claude API error: ${fallbackResponse.status}. Details: ${fallbackErrorText}`);
          }
          
          const fallbackData = await fallbackResponse.json();
          
          if (!fallbackData.content || !fallbackData.content[0] || !fallbackData.content[0].text) {
            throw new Error('Invalid response format from Claude API in fallback approach');
          }
          
          const answer = fallbackData.content[0].text;
          
          console.log("Answer generated successfully using fallback approach");
